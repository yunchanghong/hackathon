  ...
  return (
    <>
      ...
      <div className="fixed left-0 right-0 top-0 z-40 flex items-center gap-x-6 bg-white px-4 py-4 shadow-sm sm:px-6">
        ...
        >
          <span className="sr-only">打开 侧栏</span>
          <Bars3Icon className="h-6 w-6" aria-hidden="true" />
        </button>
        <div className="flex-1 text-sm font-semibold leading-6 text-gray-900 lg:pl-72">
          {props.subtitle ? (
            <>
              [大连外国语大学智慧校园]: <span className="font-normal">{props.subtitle}</span>
            </>
          ) : (
                 "👏[大连外国语大学智慧校园]👏"
          )}
        ...
        <div className="inline-flex items-center rounded-md bg-pink-100 px-2 py-1 text-xs font-medium text-pink-700">
          ChatBot/RAG/智能体 演示（每天清理数据）
        </div>
         <div className="inline-flex items-center rounded-md bg-warning px-2 py-1 text-xs font-medium text-ls-black">
          方案咨询：support@chatroller.com
        </div>
      </div>
